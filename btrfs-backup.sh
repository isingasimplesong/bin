...
cleanup_remote_snapshots() {
    ...
    for snapshot in "${snapshots[@]}"; do
        ...
        local snapshot_time=$(date -r "$snapshot" +%s)
        ...
        if [ $age -ge 7 ] && [ $weekly -lt $keep_weekly ] && [ $((snapshot_time - last_weekly_time)) -ge 604800 ]; then
            weekly=$((weekly + 1))
            last_weekly_time=$snapshot_time
            continue
        fi

        # Keep monthly snapshots
        if [ $age -ge 30 ] && [ $monthly -lt $keep_monthly ] && [ $((snapshot_time - last_monthly_time)) -ge 2592000 ]; then
            monthly=$((monthly + 1))
            last_monthly_time=$snapshot_time
            continue
        fi

        # Delete older snapshots that do not fall under daily, weekly, or monthly retention
